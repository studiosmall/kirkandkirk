...
$output .= '<div class="stockist__col">' . "\r\n";
  ...
  $output .= "\t\t\t\t" . '<input placeholder="Your Location" id="wpsl-search-input" type="text" value="' . apply_filters( 'wpsl_search_input', '' ) . '" name="wpsl-search-input" placeholder="" aria-required="true" />' . "\r\n";
  $output .= "\t\t\t" . '</div>' . "\r\n";
  if ( $wpsl_settings['radius_dropdown'] || $wpsl_settings['results_dropdown']  ) {
      ...
      if ( $wpsl_settings['radius_dropdown'] ) {
          ...
          $output .= "\t\t\t\t\t" . '<label for="wpsl-radius-dropdown">' . esc_html( $wpsl->i18n->get_translation( 'radius_label', __( 'Radius', 'wpsl' ) ) ) . '</label>' . "\r\n";
