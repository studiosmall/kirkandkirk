...
if ( ! class_exists( 'WCPBC_Product_Addons' ) ) :
	...
	class WCPBC_Product_Addons {
		...
		public static function init() {
			...
			add_action( 'woocommerce_product_addons_end', array( __CLASS__, 'addons_total_wrapper_start' ), 9 );
		...
		public static function get_cart_item_from_session( $cart_item, $values ) {
			...
			if ( self::is_product_addon_legacy() ) {
				...
				if ( isset( $cart_item['addons_price_before_calc'] ) ) {
					$cart_item['addons_price_before_calc']         = floatval( $cart_item['data']->get_price( 'edit' ) );
					$cart_item['addons_regular_price_before_calc'] = floatval( $cart_item['data']->get_regular_price( 'edit' ) );
					$cart_item['addons_sale_price_before_calc']    = floatval( $cart_item['data']->get_sale_price( 'edit' ) );
				}

				// Adjust the price.
				if ( is_callable( array( $GLOBALS['Product_Addon_Cart'], 'add_cart_item' ) ) ) {
					$cart_item = $GLOBALS['Product_Addon_Cart']->add_cart_item( $cart_item );
				}
			}
		...
		/**
		 * Product addons total wrapper for ajax geolocation.
		 */
		public static function addons_total_wrapper_start() {
			if ( is_callable( array( 'WCPBC_Ajax_Geolocation', 'is_enabled' ) ) && WCPBC_Ajax_Geolocation::is_enabled() ) {
				echo '<div class="wc-price-based-country-addons-total-wrapper' . ( defined( 'DOING_AJAX' ) && DOING_AJAX ? '' : ' loading' ) . '">';
				add_action( 'woocommerce_product_addons_end', array( __CLASS__, 'addons_wrapper_end' ), 11 );
			}
		}
		 ...
		 * @param int $product_id Product ID.
		 */
		public static function addons_wrapper_end( $product_id = 0 ) {
			echo '</div>';
		}
		...
		public static function load_scripts() {
			...
			if ( WCPBC_Ajax_Geolocation::is_enabled() && ! ( is_cart() || is_account_page() || is_checkout() || is_customize_preview() ) ) {
				wp_enqueue_style( 'wc-price-based-country-addons', WC_Product_Price_Based_Country_Pro::plugin_url() . 'assets/css/addons.css', array(), WC_Product_Price_Based_Country_Pro::$version );
			}
		}
